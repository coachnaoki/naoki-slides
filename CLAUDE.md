...
> ⚠️ ライセンスIDは naoki-blueprint と共通です（1つのIDで両方使えます）
...
3. **生徒の作業物を勝手に削除しない**: `presentations/` 配下のファイルは慎重に扱う
...
```
1. 生徒が台本.txtを作成（自由テキスト）
2. Claude Code でスキル起動（/slides-create）
3. Claudeが台本を解析 → スライド構成(JSON)を設計
