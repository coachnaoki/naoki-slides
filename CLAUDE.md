...
**すべてのスキル実行前に、必ずライセンス認証を行うこと。**

### 認証フロー（順番に試す）

1. **まず `node tools/_chk.mjs`** を実行（オンライン検証含む）
   - exit 0 → 認証済み。続行。
2. **失敗した場合、`node tools/find-license.mjs`** を実行
   - naoki-blueprint 側に `.license` があれば自動でコピー（同じPC前提）
   - 成功したら再度 `node tools/_chk.mjs`
3. **それでも失敗した場合、ユーザーにライセンスIDを聞く**
   - `node tools/validateLicense.mjs NK-XXXX-XXXX-XXXX` を実行
4. **最終的に失敗した場合（期限切れ・無効化・別PC）**: スキルの実行を中止し、「ライセンスIDが無効です。発行元に確認してください」と伝える
